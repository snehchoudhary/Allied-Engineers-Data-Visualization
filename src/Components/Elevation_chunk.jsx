...
const Elevation_Chunks = () => {
  ...
  const filtered = fileDataICE.filter(
    (item) =>
      item['Chainage (m)'] &&
      item['Elevation (m)'] &&
      item['Water Holdup (ABBL)']&&
      !isNaN(item['Chainage (m)']) &&
      !isNaN(item['Elevation (m)'])&&
      !isNaN(item['Water Holdup (ABBL)'])
  );
  ...
  for (let start = 0; start < maxDist; start += chunkSize) {
    ...
    if (chunk.length > 0) {
      ...
      const dataOff = chunk.map(item => Number(item['Water Holdup (ABBL)']));
  ...
  return (
    <div style={{padding : '2rem'}}>
      <h2>Section-wise Charts of Liquid Hold-Up for (Every {chunkSize}m)</h2>
      {chunks.map((chunk, i) => (
